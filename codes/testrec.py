...
def cal_pnsr_ssim(sr_img, gt_img):
    # save images
    suffix = opt['suffix']
    if suffix:
    ...
    if gt_img.shape[2] == 3:  # RGB image
        ...
        writer.writerow([osp.join(folder, img_name), psnr_y, ssim_y])
        logger.info(
            '{:20s} - PSNR: {:.6f} dB; SSIM: {:.6f}; PSNR_Y: {:.6f} dB; SSIM_Y: {:.6f}.'.
                format(osp.join(folder, img_name), psnr, ssim, psnr_y, ssim_y))
    else:
        writer.writerow([osp.join(folder, img_name), psnr])
        logger.info('{:20s} - PSNR: {:.6f} dB; SSIM: {:.6f}.'.format(
            osp.join(folder, img_name), psnr, ssim))
...
for test_loader in test_loaders:
    ...
    with open(osp.join(opt['path']['log'], 'test_' + opt['name'] + '_test.csv'), 'w') as f:
        ...
        for data in test_loader:
            ...
            if test_set_name == 'Vimeo90K-Test':
                print('in')
                ...
                test_results = cal_pnsr_ssim(sr_img, gt_img)
